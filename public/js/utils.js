        ...
        $.ajax({url: '/', type: 'GET', data: { index: index },contentType: 'application/json',
            success: function(data) {
                ...
                data.articles.forEach(function(article) { //console.log('Article=>',article);
                    ...
                    var upvotes1 = data.loguser && data.loguser !=='' && upvotes.includes(data.loguser) ? `<div class='hov_up'></div><div class='likes-value'>${article.likes}</div></div><div class='card-button-wrap'><span class="post-income">${postIncome}</span> TMAC</div>` :
